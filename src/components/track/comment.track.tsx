...
const CommentTrack = (props: IProps) => {
    ...
    const handleSubmit = async () => {
        const res = await sendRequest<IBackendRes<IModelPaginate<ITrackComments>>>({
                ...
                <div className="comments" style={{ flex: 1 }}>
                    {
                        comments?.map(comment => (
                            <div style={{ display: 'flex', gap: 10, justifyContent: 'space-between', margin: "24px 0" }}
                                key={comment._id}
                            >
                                <div style={{ display: 'flex', gap: 10 }}>
                                    ...
                                    <div>
                                        <div style={{ color: "#555" }}>{`${comment.user.email} at `}
                                            <span
                                                style={{ color: "rgba(255, 85, 0, 0.8)", cursor: 'pointer' }}
                                                onClick={() => handleJumpAudio(comment.moment)}
                                            >
